...
- searchParams : ?a=1&b=21로 접근할 경우 &lbrace;a:1,b:21&rbrace;이라는 자바스크립트 객체 값이 들어오게 된다. searchParams에 의존적인 작업은 반드시 page 내부에서만 수행해야 한다.
...
- cache : &lbrace;next: &lbrace;revalidate : 10 &rbrace;&rbrace; : 정해진 기간동안 캐싱하고 그 이후에는 캐싱을 파기한다.
